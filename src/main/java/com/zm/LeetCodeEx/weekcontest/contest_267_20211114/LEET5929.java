 ...
 *
 */
public class LEET5929 {
    public static void main(String[] args) {
        LEET5929 leet5928 = new LEET5929();
        System.out.println(JSON.toJSONString(leet5928.new Solution()
    ...
    class Solution {
        public boolean[] friendRequests(int n, int[][] restrictions, int[][] requests) {
            ...
            for (int i = 0; i < requests.length; i++) {
                int[] ufTemp = uf.clone();
                union(requests[i][0], requests[i][1], ufTemp);
                for (int[] r : restrictions) {
                    if (isGroup(r[0], r[1], ufTemp)) {
                        continue loop1;
                    }
                }
                res[i] = true;
                union(requests[i][0], requests[i][1], uf);
            }
            return res;
        }

        private int getParent(int x, int[] temp) {
            while (temp[x] != x) {
                x = temp[x];
            }
            return x;
        ...
        private void union(int x, int y, int[] temp) {
            int px = getParent(x, temp);
            int py = getParent(y, temp);
            if (px == py) {
                return;
            } else if (px < py) {
                temp[py] = px;
        ...
        private boolean isGroup(int x, int y, int[] temp) {
            return getParent(x, temp) == getParent(y, temp);
        }
