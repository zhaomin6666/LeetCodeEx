...
public class LEET011 {
    public static void main(String[] args) {
        ...
        System.out.println(l011.new Solution4().maxArea(new int[]{2, 3, 10, 5, 7, 8, 9}));
        System.out.println(l011.new Solution4().maxArea(new int[]{1, 8, 6, 2, 5, 4, 8, 3, 7}));
    }
    ...
    /**
     * 优化上面的双指针，由于两条边是取决于短边，
     * 所以在把长边向中间移动的时候如果还是长于短边那么能够肯定面积没有原来大，直接使用while循环使指针快速移动
     */
    public class Solution4 {
        public int maxArea(int[] height) {
            ...
            while (left < right) {
                ...
                int volumn = nowHeight * (right - left);
                if (volumn > max) {
                    max = volumn;
                }
                if (height[left] > height[right]) {
                    int nowRight = height[right];
                    while (height[right] <= nowRight && right > left) {
                        right--;
                    }
                } else {
                    int nowLeft = height[left];
                    while (height[left] <= nowLeft && right > left) {
                        left++;
                    }
                }
