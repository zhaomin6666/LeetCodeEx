...
public class LEET5354 {
    ...
    class Solution {
        private int informedStaff = 1;
        private int n;
        private HashMap<Integer, LinkedList<Integer>> allBelongStaff;
        private int[] informTime;

        public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
            this.n = n;
            this.informTime = informTime;
            allBelongStaff = new HashMap<>();
            for (int i = 0; i < manager.length; i++) {
                LinkedList<Integer> list = allBelongStaff.getOrDefault(manager[i], new LinkedList<>());
                list.add(i);
                allBelongStaff.put(manager[i], list);
            }

            int minutes = informStaff(headID);
            return minutes;
        }

        private int informStaff(int headID) {
            if (informedStaff == n) {
                return 0;
            }
            int maxUseTime = 0;
            LinkedList<Integer> belongStaff = allBelongStaff.get(headID);
            if (belongStaff == null) {
                return 0;
            }
            for (Integer i : belongStaff) {
                int useTime = informStaff(i);
                maxUseTime = useTime > maxUseTime ? useTime : maxUseTime;
            }
            return informTime[headID] + maxUseTime;
        }
    }
}
